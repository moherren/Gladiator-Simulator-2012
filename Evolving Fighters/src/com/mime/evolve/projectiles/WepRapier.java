...
public class WepRapier extends SwingProjectile{
	...
	public void newProjectile(Player p,boolean gene[]){
		...
		if(gene[2]||gene[3]){
			...
			brothers.add(game.createProjectile(new StabProjectile(p.x+size*2,p.y,p.direction,game.getEnemy(p),p,size,speed/10.000,endTime,damage,brothers)));
			brothers.add(game.createProjectile(new StabProjectile(p.x+size*4,p.y,p.direction,game.getEnemy(p),p,size,speed/10.000,endTime,damage,brothers)));
			brothers.add(game.createProjectile(new StabProjectile(p.x+size*6,p.y,p.direction,game.getEnemy(p),p,size,speed/10.000,endTime,damage,brothers)));
			brothers.add(game.createProjectile(new StabProjectile(p.x+size*8,p.y,p.direction,game.getEnemy(p),p,size/2,speed/10.000,endTime,damage,brothers)));
		}
	}
}
